...
# 音声出力終了後、スピーカーの残響を拾わないための待機時間
MIC_RESUME_DELAY_SECONDS = 0.4
    ...
    try:
        with sd.InputStream(
            ...
            while not stop_event.is_set():
                # VOICEVOX再生が始まったら、このInputStreamを閉じて
                # マイク入力を完全に一時停止する。
...
class CcVoiceApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        # VOICEVOX再生中はセットされ、音声入力スレッドがマイクを開かない
        self.voice_output_active_event = threading.Event()
    ...
    def voice_input_worker(self, device_index: int) -> None:
        ...
        try:
            while (
                ...
                # VOICEVOX再生中はマイクを開かずに待機する
                while (
                    self.voice_output_active_event.is_set()
                    and not self.voice_input_stop_event.is_set()
                ):
                    time.sleep(0.05)
                ...
                wav_bytes = record_until_silence(
                    ...
                    pause_event=self.voice_output_active_event,
                )
    ...
    def stop_monitoring(self) -> None:
        self.stop_event.set()
        self.voice_output_active_event.clear()
        self.voice_input_stop_event.set()
        self.voice_input_var.set(False)
    ...
    def speak_character_message(
        ...
        # このキャラクターの音声出力処理中はマイク入力を停止する。
        # すでにInputStreamが開いている場合も、
        # record_until_silence側がこのイベントを検知して閉じる。
        self.voice_output_active_event.set()
