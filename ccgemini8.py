    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                # 取得した回答を順番にココフォリアへ連投する
                for answer_number, gemini_answer in enumerate(
                    gemini_answers,
                    start=1
                ):
                    # 回答なしの場合は投稿しない
                ...
                if sent_any_message:
                    write_log(
                        f"Geminiの回答を{len(gemini_answers)}件、"
                        "ココフォリアへ投稿しました。"
                    )
