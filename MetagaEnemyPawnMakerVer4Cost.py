...
class GuardianData():
    ...
    def input_data(self, level=5, guardian_type="ソロ", guardian_class="ミーレス（カバリエ）"):
        ...
        if self.guardian_type == "モブ":
            self.outfits_total_fp = max(10, (random.randint(self.level - 2, self.level) * 10) + (random.randint(1, 10) * 1))
        elif self.guardian_type == "ソロ":
            self.outfits_total_fp = max(5, (random.randint(self.level - 2, self.level) * 7) + (random.randint(1, 7) * 1))
        elif self.guardian_type == "強敵":
            self.outfits_total_fp = max(50, (random.randint(self.level - 2, self.level) * 20) + (random.randint(1, 20) * 1))
