...
# Geminiの回答がトークン上限で途切れた場合の最大連投数
# 1回目の回答を含むため、10なら最大10発言まで投稿する
MAX_CONTINUATION_MESSAGES = 10

# 続きをGeminiへ依頼するときの指示
CONTINUATION_PROMPT = (
    "直前の返答が途中で途切れています。"
    "重複や前置きを入れず、途切れた箇所の直後から続きを出力してください。"
)
...
def get_finish_reason(response) -> str:
    ...
    try:
        ...
        # "FinishReason.MAX_TOKENS"のような形式にも対応
        return (
            str(finish_reason)
            .split(".")[-1]
            .upper()
        ...
        write_log(
            f"Gemini終了理由の取得に失敗しました: {error}"
        )

        return "UNKNOWN"


def send_one_message_to_gemini(
    chat,
    message: str
):
    """
...
def send_to_gemini(
    ...
    回答がMAX_TOKENSで途中終了した場合は、
    続きが自然終了するまで自動的に取得する。

    取得した複数の回答はすべて結合し、
    ココフォリアへ1回で投稿できる1つの文字列として返す。
    """
    ...
    for message_number in range(
        1,
        MAX_CONTINUATION_MESSAGES + 1
    ):
        response = send_one_message_to_gemini(
            chat,
            next_message
        )
        ...
        write_log(
            f"Geminiの返答{message_number}: {answer}"
        )

        write_log(
            f"Geminiの終了理由{message_number}: "
            f"{finish_reason}"
        )
        ...
        if message_number >= MAX_CONTINUATION_MESSAGES:
            write_log(
                "Geminiの回答がまだ続く可能性がありますが、"
                "無限取得を防ぐため最大継続回数で停止しました。"
            )
            break
    ...
    # 続きは「直前の途切れた箇所の直後から」出すよう指示しているため、
    # 原則として改行を挟まず、そのまま連結する。
    combined_answer = "".join(answer_parts).strip()

    write_log(
        f"Geminiの分割回答{len(answer_parts)}件を結合しました。"
    )

    write_log(
        f"結合後のGemini返答: {combined_answer}"
    )
...
def get_ccfolia_messages(
    driver: WebDriver
) -> list[str]:
    """
    ココフォリアに現在表示されているメッセージを
    表示順のリストで取得する。

    監視開始時の既存メッセージ数を基準位置として記録し、
    その後に追加された発言だけを処理するために使用する。
    """
...
def get_latest_ccfolia_message(
    ...
    最新のメッセージを取得する。
    """
    messages = get_ccfolia_messages(driver)
...
def should_reply(
    ...
    if (
        character_response_word2
        and character_response_word2 in original_text
    ):
        random_number = 0
...
def main() -> None:
    ...
    try:
        ...
        if character_name:
            ...
            character_response_word = "全員応答"
            character_response_word2 = "#全員"
        ...
        # 「アクティブ状態」が表示された時点までの既存発言を
        # すべて読み飛ばすための基準位置。
        #
        # ここで現在表示されている発言数を記録しておくことで、
        # その後に画面をクリック・スクロールしても、
        # 起動前から存在した発言を「新しい発言」と誤認しない。
        initial_messages = get_ccfolia_messages(
            driver_cc
        )
        next_message_index = len(initial_messages)

        if initial_messages:
            before_text = initial_messages[-1]

        write_log(
            "アクティブ状態になりました。"
            f"既存発言{next_message_index}件を読み飛ばし、"
            "これ以降に追加された発言から会話を開始します。"
        )
        ...
        while True:
            ...
            try:
                ...
                # 表示中の履歴件数が基準位置より減った場合は、
                # 画面側で履歴の再描画・絞り込み等が起きた可能性がある。
                # この場合、既存発言を再読込しないことを優先して、
                # 現在の末尾を新しい基準位置にする。
                if len(messages) < next_message_index:
                    next_message_index = len(messages)

                    if messages:
                        before_text = messages[-1]

                    write_log(
                        "表示中の発言一覧が再構成されたため、"
                        "現在表示されている発言を既存履歴として読み飛ばします。"
                    )
                ...
                # 基準位置より後ろに追加された最初の発言だけを取得する。
                # 複数発言が短時間に追加された場合も、次のループで順番に処理する。
                latest_text = messages[next_message_index]
                ...
                # 結合済みの回答をココフォリアへ1回だけ投稿する
                if send_message_to_ccfolia(
                    driver_cc,
                    output_text
                ):
                    # 自分が投稿した最新文章を監視対象から除外する。
                    before_text = output_text

                    # indexベースの監視でも自分の投稿を新規発言として
                    # 読み込まないよう、現在表示されている末尾まで進める。
                    current_messages = get_ccfolia_messages(
                        driver_cc
                    )
                    next_message_index = len(current_messages)
